...
function expectAst(query: string, ast: Ast) {
...
test("english", () => {
  ...
  expectAst("Hello", ["Hello"]);
  expectAst("this. is - definitely  test ~ ", [
    "this",
    "is",
    "definitely",
    "test",
...
test("romaji", () => {
  ...
  expectAst("red jagaimo. something", [
    "red",
    ["jagaimo", "じゃがいも"],
    "something",
  ]);
...
test("tokenize", () => {
  expect(
    tokenize(
      "this.is 123 some english! Also がいこくじん。「にほんじん」かのじょたち ! and スーパー with some  ファックス。 「エスカレーター」ワイシャツ $ lets not forget 必須条件 and --- a bit of じゃが芋",
    ),
  ).toStrictEqual([
    { type: "en", value: "this" },
    { type: "en", value: "is" },
