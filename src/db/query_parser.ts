...
import { isKatakana as wkIsKatakana, toKana, tokenize as wkTokenize } from "wanakana";

function parseQueryMiniSearch(input: string): Query {
  const ast = parseToAst(input);

  return { combineWith: 'AND', queries: ast.map((element) => {
...
export function tokenize(input: string): Token[] {
  const wkTokens = wkTokenize(input, { compact: true, detailed: true }) as Token[];
  const tokens: Token[] = [];

  const isKatakana = (t: Token | undefined) => t?.type === "ja" && wkIsKatakana(t.value);

  for (const token of wkTokens) {
    const prevToken = tokens[tokens.length - 1];
    
    if (token.type === "other" && token.value.startsWith("ー") && isKatakana(prevToken)) {
      ...
      if (rest.length > 0) {
        tokens.push({ ...token, value: token.value.slice(katakanaPart.length) })
      }
  ...
  return tokens.map((t) => { return { ...t, value: t.value.trim() } }).filter((t) => t.type !== "other" && t.value.length > 0);
}
